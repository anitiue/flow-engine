...
- **当前发布形态**：① Hanako 插件版（含自动保障层）② MCP 服务版（跨软件通用，见下）；
- **核心通用性**：流程工具本身不绑定任何平台，已提供 MCP 封装（`mcp/`），供 Claude、Codex、Cursor 等任意支持 MCP 的 AI 软件使用（见"跨软件使用"）。

## 两种形态怎么选

| | Hanako 插件版 | MCP 服务版 |
|---|---|---|
| 适用环境 | Hanako（已验证 0.323.0） | 任意支持 MCP 的 AI 软件 |
| 流程工具（15 个） | ✅ 全量 | ✅ 全量 |
| 自动保障（每轮注入 / 派发拦截 / 自动贴验收标准） | ✅ 全自动，零配置 | ❌ 无（宿主无事件钩子） |
| 使用方式 | 安装即用 | 连接即用（`node mcp/server.js`，stdio） |
| 数据目录 | `plugin-data/flow-engine/` | `~/.flow-engine/`（`FLOW_ENGINE_DATA_DIR` 可改） |

**追求效果最好、零配置 → 用 Hanako 插件版；追求跨软件通用 → 用 MCP 服务版。** 本地 Hanako 建议使用插件版。
...
- **MCP 形态**：任何支持 MCP 协议的 AI 软件（Claude Desktop / Claude Code / Codex / Cursor / Windsurf 等）都能连接并使用全部 15 个流程工具（同一套核心，工具层功能一致）；
- **CLI 形态**：可在任意具备 shell 权限的 agent 环境中直接调用；
- **自行封装**：核心为零依赖的纯函数 + 文件存储，无任何平台绑定——任何开发者都可用官方 MCP SDK 直接包装为 MCP 服务（约百行级工作，无需改动核心代码），即可暴露全部流程工具；
- **功能差异（重要）**：自动保障层（每轮状态注入、派发拦截、自动附加验收标准）依赖 Hanako 的事件钩子，目前为 Hanako 独有能力。其他软件接入时仅获得流程工具本身（建单、门禁、签名、归档），"自动强制走流程"需要宿主提供类似的事件机制。

作者可提供 MCP 封装版本，需要时在仓库讨论区提出即可。
